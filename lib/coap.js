...
class AirPurifier extends EventEmitter {
 ...
 /*
    philips-air.0 	2021-10-03 11:23:25.105	error	req.abort is not a function
    philips-air.0 	2021-10-03 11:23:25.101	error	TypeError: req.abort is not a function at OutgoingMessage.<anonymous> (/opt/iobroker/node_modules/iobroker.philips-air/lib/coap.js:118:21) at OutgoingMessage.emit (node:events:394:28) at RetrySend.emit (node:events:394:28) at Timeout._onTimeout (/opt/iobroker/node_modules/iobroker.philips-air/node_modules/coap/lib/retry_send.js:77:12) at listOnTimeout (node:internal/timers:557:17) at processTimers (node:internal/timers:500:7)
    philips-air.0	2021-10-03 11:23:25.089	error	uncaught exception: req.abort is not a function
*/


    constructor(deviceIp, options, parent) {
        super();
        ...
        this.parent.log.info("constructor called");
    ...
    sync() {
        const syncRequest = [0, 0, 0, 0].map((a, i) => Math.round(Math.random() * 0xFF).toString(16).padStart(2, '0')).join('').toUpperCase();
        return this._post('/sys/dev/sync', syncRequest)
            .then(clientKey => this.clientKey = clientKey.toString());
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            req.on('response', res => {
                ...
                res.on('error', err => {
                    ...
                    this.parent.log.error('Error by receiving: ' + err);
                    res.close();
                    reject(err);
                });
                ...
                res.on('end', function() {
                    ...
                    //this.parent.log.debug('Received: ' + received.toString());
                    resolve(received);
                })
            });
            req.on('error', err => {
                console.error('Error by sending: ' + err);
                this.parent.log.error('Error by sending: ' + err);
                req.abort();
                reject(err);
            });
    ...
    _reconnect() {
        ...
        this.sync()
            .then(clientKey => this._subscribeOnStatus());
    }
                    ...
                    this.decryptPayload(chunk)
                        .then(status => {
                            try {
                                ...
                                this.parent.log.error('Cannot parse: ' + status);
    ...
    decryptPayload(encryptedData) {
        ...
        if (digest !== hash) {
            this.parent.log.error('Message corrupted');
    ...
    updateClientKey() {
        ...
        this.parent.log.debug('Update client key: ' + this.clientKey);
    }
    ...
    control(settings) {
        ...
        Object.keys(NAME_MAPPING).filter(attr => NAME_MAPPING[attr].control).forEach(attr => {
            if (settings.hasOwnProperty(NAME_MAPPING[attr].name)) {
        ...
        return this.encryptPayload(payload)
            ...
            .then(data => {
                try {
                    ...
                    this.parent.log.error('Cannot parse: ' + data);
                    console.error('Cannot parse: ' + data);
                }
                this.parent.log.debug('Data: ' + JSON.stringify(data));
        ...
        if (status && status.state && status.state.reported) {
            Object.keys(status.state.reported).forEach(attr => {
                if (NAME_MAPPING[attr]) {
                    ...
                    if (NAME_MAPPING[attr].options && NAME_MAPPING[attr].options.hasOwnProperty(val)) {
