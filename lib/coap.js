...
class AirPurifier extends EventEmitter {
    ...
    // Safely tear down the current observe request. reset() can throw "Empty messages must be empty"
    // (node-coap issue #175) when the observe already received data; swallow it so callers are not
    // disrupted by a failed cleanup.
    _closeStatusRequest() {
        if (this._statusRequest) {
            try {
                this._statusRequest.reset();
            } catch (err) {
                this.emit(
                    'debug',
                    `Could not reset previous observe request: ${err && err.message ? err.message : err}`,
                );
            }
    ...
    _post(path, payload) {
        return new Promise((resolve, reject) => {
            ...
            req.on('error', err => {
                ...
                // The coap request object has no abort()/reset() at this point, so we just reject and let
                // the reconnect timer reissue the request. Previously the promise was left pending forever,
                // which blocked sync()/control() and leaked the request. Callers must catch the rejection.
                if (!settled) {
                    settled = true;
                    reject(err);
                }
