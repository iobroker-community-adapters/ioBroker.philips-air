...
// Ceiling for the reconnect backoff: a device that stays unreachable is retried every 5 minutes
// instead of every reconnectInterval, which keeps both the network and the log quiet.
const MAX_RECONNECT_INTERVAL = 300000;
...
class AirPurifier extends EventEmitter {
    ...
    async _reconnect() {
        ...
        this._scheduleRetry(this.reconnectInterval);
     ...
     *
     * So ask the device first. Only the FAILURE of that probe triggers a reconnect - which is exactly
     * what used to happen unconditionally, so firmwares that never answer /sys/dev/sync (e.g. AC2889
     * fw 1.0.7) behave as before and cannot flap: a successful probe can only prevent a reconnect,
     * never cause one. The probe costs one round trip per staleTimeout and refreshes the command key
     * on the way; a failing one delays the reconnect by at most aliveTimeout.
     */
    async _checkAlive() {
        if (this.destroyed) {
            return;
        ...
        // The observe request stays untouched - only the watchdog is re-armed. _markConnected() emits
        // nothing while the session is already marked connected, so a quiet device stays quiet in the log.
        this.emit('debug', `${this.deviceIp} answered the probe, keeping the observe stream open`);
        this._markConnected(`Connected to ${this.deviceIp} using CoAP`);
    }
                    ...
                    decryptPromise
                        .then(status => {
                            ...
                            if (parsed && parsed.state && parsed.state.reported) {
                                this.emit('status', parsed.state.reported);
                            }
                        })
                        .catch(err => {
                            this.emit(
