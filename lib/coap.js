...
class AirPurifier extends EventEmitter {
    ...
    async _reconnect() {
        ...
        try {
            // Subscribe first: receiving status via the observe does not need a freshly synced key, and
            // this is the path that actually proves the device is reachable. Only a failing subscribe
            // means the device is truly gone.
            this.emit('debug', `Connecting to ${this.deviceIp} (CoAP): subscribing to status...`);
            await this._subscribeOnStatus();
            // Negotiate a control key as a best-effort follow-up. A failing/timing-out /sys/dev/sync
            // (some firmwares never answer it while still pushing status) must not tear the connection
            // down - we keep the previous key until the next sync succeeds; only control commands wait.
            try {
                await this.sync();
                this.emit('debug', `Key synced (${this.clientKey || 'empty'})`);
            } catch (syncErr) {
                this.emit(
                    'debug',
                    `Key sync failed, control may be delayed: ${syncErr && syncErr.message ? syncErr.message : syncErr}`,
                );
            }
        } catch (err) {
