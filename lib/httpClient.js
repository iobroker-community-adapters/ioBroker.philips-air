...
// dependency chain in favour of the built-in node:crypto and node:http.
const G =
...
class HttpClient {
     ...
     * @param {Buffer|null} [key] previously negotiated AES key buffer (optional)
     */
    ...
    /**
     * Perform one request against the device and return its body.
     *
     * Deliberately built on node:http instead of the global fetch API. The embedded web server in
     * these purifiers rejects what undici puts on the wire: it always adds accept-encoding,
     * accept-language and sec-fetch-mode and writes every header name in lower case, none of which
     * can be switched off. Devices that answer fine on 1.3.0 fail from 1.4.0 on with
     * UND_ERR_SOCKET (issue #383). The header set below is the one used by the two clients known
     * to work with these devices: axios 0.20 (adapter <= 1.3.0) and Python urllib
     * (py-air-control). Keep it minimal - do not add headers here without testing on real hardware.
     *
     * @param endpoint device endpoint path
     * @param options method, request body and its content type
     * @returns the response body as a string
     */
    _request(endpoint, options = {}) {
        const { method = 'GET', body = null, contentType = null } = options;
        const url = new URL(`http://${this.host}${endpoint}`);
        return new Promise((resolve, reject) => {
            /** @type {Record<string, string|number>} */
            const headers = { Accept: 'application/json, text/plain, */*', Connection: 'close' };
            if (body !== null) {
                ...
                response => {
                    ...
                    response.on('end', () => {
                        ...
                        if (status < 200 || status > 299) {
                            reject(
                                new Error(`Unexpected response ${status} ${response.statusMessage} for ${endpoint}`),
                            );
                            return;
                        }
                        resolve(data);
            ...
            // node:http's own `timeout` option only fires on socket inactivity, while the previous
            // fetch implementation had a hard deadline for the whole request. Keep that guarantee.
            deadline = setTimeout(
                () => request.destroy(new Error(`No response from ${this.host} for ${method} ${endpoint}`)),
                this.timeout,
            );
    ...
    async getKey() {
        ...
        dh.generateKeys();
        const body = await this._request('/di/v1/products/0/security', {
            method: 'PUT',
            body: JSON.stringify({ diffie: dh.getPublicKey('hex') }),
            contentType: 'application/json',
        });
        const data = /** @type {{ hellman: string, key: string }} */ (JSON.parse(body));
     ...
     * @returns the decrypted device response, or an empty string if the device answered with an empty body
     */
    async setValues(values) {
        ...
        const body = await this._request('/di/v1/products/1/air', { method: 'PUT', body: encrypted });
        // Devices answer a control PUT with the updated status - encrypted like every other payload.
        // Some firmwares reply with an empty body instead, so an empty answer is not an error.
        return body ? decrypt(body, this.key) : '';
    }
    ...
    async getOnce(endpoint) {
        ...
        return decrypt(await this._request(endpoint), this.key);
    }
